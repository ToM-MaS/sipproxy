...
class DbaliasTest < ActiveSupport::TestCase
  ...
  should "not be valid with alias_username == username and alias_domain == domain" do
    assert ! Factory.build( :dbalias, {
      :username       => 'test_user',
      :domain         => 'test_domain',
      :alias_username => 'test_user',
      :alias_domain   => 'test_domain',
    }).valid?
  end
  ...
  should "not be valid if not unique" do
    ...
    assert ! Factory.build( :dbalias, {
      :username       => dbalias.username,
      :domain         => dbalias.domain,
      :alias_username => dbalias.alias_username,
      :alias_domain   => dbalias.alias_domain,
    }).valid?
  end
